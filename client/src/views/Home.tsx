...
const Home = () => {
    ...
    const handleLoginUser = async (event : React.FormEvent<HTMLFormElement>) => {
        ...
        if (response.status ===  200) { navigate('/campaigns')}
    ...
    return (
        ...
        <input id="login-input" name="login" type='text' placeholder="Podaj login"/>
          </div>
          <div className='password-wrapper'>
        <label htmlFor="password-input">Hasło:</label>
        <input id="password-input" name="password" type='password' placeholder="Podaj hasło"/>
    </div>
