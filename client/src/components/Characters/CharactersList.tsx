...
import { addCharacter, fetchCharactersForCampaign, fetchClasses, fetchRaces, updateCharacter } from "../../api/characterApi";
...
const CharacterList : React.FC<CharacterListProps> = ({playersNum}) => {
    ...
    return (
        ...
        {   [...Array(playersNum)].map((_, index) => {
            ...
            races={races}
            classes={classes}
            onClick={() => {console.log("sada" + !characters[index]);setSCharacter(characters[index] || null); setCreatingCharacter(true)}}/>
        })
        }

        {creatingCharacter && (<CharacterForm character={scharacter} races={races} classes={classes} onSubmit={handleCharacterSubmit} onCancel={handleCharacterCancel}/>)}
