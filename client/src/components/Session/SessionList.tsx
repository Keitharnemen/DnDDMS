...
const SessionList: React.FC = () => {
  ...
  const getSessions = async () => {
    ...
    if (response.status === 200) {
      const data: ISession[] = response.data;
      console.log(data);
      setSessions(data);
  ...
  const postSession = async (name: string) => {
    ...
    if (response.status === 201) {
      const newSession: ISession = response.data;
  ...
  const handleSelectSession = async (
    ...
    if (response.status === 200) {
      navigate("/campaigns/sessions/details", {
        state: { name: details.name, plan: details.plan, notes: details.notes },
      });
    } else {
      setError({
        status: response.status,
