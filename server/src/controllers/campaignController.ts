...
export const getCampaigns = async (req: Request, res: Response) => {
  try {
    ...
    if (!DMID) {
      res.status(401).json({ message: "No Master ID" });
      return;
    }
...
export const addCampaigns = async (req: Request, res: Response) => {
  try {
    ...
    if (!name || !playersNum || !system) {
      res
        .status(400)
        .json({
          message: "Invalid input: no Name, Number of Players or System",
        });
      return;
    }
...
export const updateCharacter = async (req: Request, res: Response) => {
  try {
    ...
    const dbCharacter = await CharacterModel.findOneAndUpdate(
      { id: character.id },
      { ...character },
      { new: true }
    );
