...
export const addSession = async (req: Request, res: Response) => {
  try {
    ...
    const { name } = req.body;
    if (!name) {
      res.status(400).json({ message: "Invalid input: no name" });
      return;
    }
...
export const changeSessionID = (req: Request, res: Response) => {
  try {
    ...
    if (!sessionID) {
      res.status(400).json({ message: "Invalid input: no sessionId" });
      return;
    }
...
export const updateSessionData = async (req: Request, res: Response) => {
  try {
    ...
    if (!plan || !notes) {
      res.status(400).json({ message: "Invalid input: no plan or notes" });
      return;
    }
